...
function createVirtualTables {
  ...
  createTable "virtual_mailboxes" "CREATE TABLE IF NOT EXISTS virtual_mailboxes (id SERIAL PRIMARY KEY, username VARCHAR(255) NOT NULL UNIQUE, maildir VARCHAR(255) NOT NULL);"
  createTable "virtual_users" "CREATE TABLE IF NOT EXISTS virtual_users (id SERIAL PRIMARY KEY, username VARCHAR(255) NOT NULL UNIQUE, password TEXT NOT NULL);"
}
...
function insertInitialData {
  ...
  local insert_sql="
    ...
    INSERT INTO virtual_users (username, password) VALUES ('info@mail.smartquail.io', 'ms95355672') ON CONFLICT DO NOTHING;
    INSERT INTO virtual_aliases (source, destination) VALUES ('info', 'info@mail.smartquail.io') ON CONFLICT DO NOTHING;
    INSERT INTO virtual_mailboxes (username, maildir) VALUES ('info@mail.smartquail.io', '/var/mail/info') ON CONFLICT DO NOTHING;
